...
def openExpoFiles():
    ...
    document2 = xlrd.open_workbook("Tags_Musee_Agde.xls")
...
def threadTrack():
    global istep, pts, flag_update, ax, w
    istep = istep + 1    
...
#    Orientation=np.arccos(magZnorm)*180 / np.pi;
    ...
    for k in range (0, len(pointFinal)):
        if pointFinal[k]>0:
            coord += [[feuille_2.cell_value(rowx=int(pointFinal[k])-1, colx=1), feuille_2.cell_value(rowx=int(pointFinal[k])-1, colx=2)]]       
    pts = np.array(coord)
...
def plotTravel():
    global root, ax, Figscatter, istep , w          
    istep=0
    threadTrack()
...
root = tk.Tk()
print(istep)
slider=tk.Scale(root,from_=0,to=1000, orient='horizontal',command=threadTrack2)
slider.pack()

#Personalisation
#root.title("Scenoscope")
#root.geometry("980x620")
#root.iconbitmap("sceno_home.ico")
